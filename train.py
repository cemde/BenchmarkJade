...
def main(args: argparse.Namespace):
    ...
    for i, (inputs, labels) in enumerate(val_loader):
        ...
        if i % 100 == 0 and i > 0:
            fabric.print(
                f"Batch {i+1}: Data Loading Time: {data_loading_times[-1]:.4f} seconds, Forward Pass Time: {forward_pass_times[-1]:.4f} seconds, Backward Pass Time: {backward_pass_times[-1]:.4f}, Batch Size: {len(inputs)}"
            )
    ...
    utils.print_benchmark_results(
        fabric, args, data_loading_times, [0], forward_pass_times, backward_pass_times, loop_time
    )
