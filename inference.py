...
def main(args: argparse.Namespace):
    ...
    with torch.no_grad():
        for i, (inputs, labels) in enumerate(val_loader):
            ...
            if i % 100 == 0 and i > 0:
                fabric.print(f"Batch {i+1}: Data Loading Time: {data_loading_times[-1]:.4f} seconds, Forward Pass Time: {forward_pass_times[-1]:.4f} seconds. Batch Size: {len(inputs)}")
    ...
    return np.mean(data_loading_times), 0, np.mean(forward_pass_times), 0
